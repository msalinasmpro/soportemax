...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabase.from('brands').select('*').eq('active', true).order('sort_order');
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await supabaseAdmin.from('brands').update(fields).eq('id', id).select().single();
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json(data);
  }
