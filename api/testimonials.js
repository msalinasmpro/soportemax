...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await supabaseAdmin.from('testimonials').update(fields).eq('id', id).select().single();
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json(data);
  }

  if (req.method === 'DELETE') {
    const { error } = await supabaseAdmin.from('testimonials').delete().eq('id', req.query.id);
    if (error) return res.status(500).json({ error: error.message });
