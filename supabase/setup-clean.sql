CREATE TABLE IF NOT EXISTS site_config (key TEXT PRIMARY KEY, value JSONB NOT NULL DEFAULT '{}'::jsonb, updated_at TIMESTAMPTZ DEFAULT now());
ALTER TABLE site_config ENABLE ROW LEVEL SECURITY;
CREATE POLICY "c1" ON site_config FOR SELECT USING (true);
CREATE POLICY "c2" ON site_config FOR ALL USING (auth.role() = 'authenticated');

CREATE TABLE IF NOT EXISTS services (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), title TEXT NOT NULL, description TEXT, icon TEXT, image_url TEXT, sort_order INT DEFAULT 0, active BOOLEAN DEFAULT true, created_at TIMESTAMPTZ DEFAULT now());
ALTER TABLE services ENABLE ROW LEVEL SECURITY;
CREATE POLICY "s1" ON services FOR SELECT USING (active = true);
CREATE POLICY "s2" ON services FOR ALL USING (auth.role() = 'authenticated');

CREATE TABLE IF NOT EXISTS testimonials (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), name TEXT NOT NULL, role TEXT, comment TEXT, rating INT DEFAULT 5, avatar_url TEXT, active BOOLEAN DEFAULT true, created_at TIMESTAMPTZ DEFAULT now());
ALTER TABLE testimonials ENABLE ROW LEVEL SECURITY;
CREATE POLICY "t1" ON testimonials FOR SELECT USING (active = true);
CREATE POLICY "t2" ON testimonials FOR ALL USING (auth.role() = 'authenticated');

CREATE TABLE IF NOT EXISTS faqs (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), question TEXT NOT NULL, answer TEXT, sort_order INT DEFAULT 0, active BOOLEAN DEFAULT true, created_at TIMESTAMPTZ DEFAULT now());
ALTER TABLE faqs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "f1" ON faqs FOR SELECT USING (active = true);
CREATE POLICY "f2" ON faqs FOR ALL USING (auth.role() = 'authenticated');

CREATE TABLE IF NOT EXISTS gallery (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), title TEXT, image_url TEXT NOT NULL, sort_order INT DEFAULT 0, active BOOLEAN DEFAULT true, created_at TIMESTAMPTZ DEFAULT now());
ALTER TABLE gallery ENABLE ROW LEVEL SECURITY;
CREATE POLICY "g1" ON gallery FOR SELECT USING (active = true);
CREATE POLICY "g2" ON gallery FOR ALL USING (auth.role() = 'authenticated');

CREATE TABLE IF NOT EXISTS contact_messages (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), name TEXT NOT NULL, email TEXT NOT NULL, phone TEXT, message TEXT NOT NULL, read BOOLEAN DEFAULT false, created_at TIMESTAMPTZ DEFAULT now());
ALTER TABLE contact_messages ENABLE ROW LEVEL SECURITY;
CREATE POLICY "m1" ON contact_messages FOR INSERT WITH CHECK (true);
CREATE POLICY "m2" ON contact_messages FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "m3" ON contact_messages FOR UPDATE USING (auth.role() = 'authenticated');
